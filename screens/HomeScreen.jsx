...
const HomeScreen = ({ navigation }) => {
  ...
  const Category = ({ onPress, item }) => (
    ...
    >
      ...
      >
        {item?.icon?.url ? (
          <CategoryImage size={50} uri={item.icon.url} />
        ) : (
          <CategoryIcon
          ...
          <>
            ...
            >
              {topCategoriesData.map((cat, i) => (
                <Category key={i} item={cat} onPress={handleSelectCategory} />
              ))}
            </View>
          </>
        )}
  ...
  return (
    <View style={styles.container}>
      {ios && admobConfig?.admobEnabled && (
        <Modal
          animationType="slide"
          transparent={true}
          visible={!admobPermission}
        >
          <View style={styles.admobOverLay}>
            <Text style={styles.admobOverLayText}>
              {__("adMobTexts.appDisabledText", appSettings.lng)}
            </Text>
          </View>
        </Modal>
      )}
